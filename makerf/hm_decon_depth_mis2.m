function [bma_list,stt_list,edt_list] = hm_decon_depth_mis2(rf,fn,evtime,sel_data,z_out,ldepth,velp,vels,fcut,z_targets)
...
for i = 1:length(evtime)
    bootg = zeros(10,10,sel_data.nf);
    bootd = zeros(10,sel_data.nf);
    ...
    bootg = bootg+bsxfun(@times,bfdyadr,reshape(abs(rf.(fn{(i)}).rstd(1).data).^2,1,1,numel(rf.(fn{(i)}).rrf(1).data)));
    bootd = bootd+bfr'*rf.(fn{(i)}).rrf(1).data';
    ...
    bootg = bootg+bsxfun(@times,bfdyadt,reshape(abs(rf.(fn{(i)}).tstd(1).data).^2,1,1,numel(rf.(fn{(i)}).trf(1).data)));
    bootd = bootd+bft'*rf.(fn{(i)}).trf(1).data';
...
for i = 1:length(et_list)
    ...
    keyboard
    for ii = 1:10
        temp = fftshift(real(ifft((ctap'.*bootma(ii,:)))));
        tt = sel_data.dt*((1:length(temp))-length(temp)/2)';
        bootma2(ii,:) = depthmapping(tt,temp,z_out,ldepth,velp,vels,slow);
    end
    
    bma_list{m} = real(bootma2);
