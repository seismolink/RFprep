function bdsr_makerf(path2opts,sel_data)
...
sel_data.load_dir = [sel_data.work_dir '/processed/'];
...
for n = 1:length(station)
    rfchk = tic;
    if sel_data.prfflag
        makerf_bdsr(sel_data,nc{n},station{n});
    elseif sel_data.srfflag
        makesrf_bdsr(sel_data,nc{n},station{n});
    end
