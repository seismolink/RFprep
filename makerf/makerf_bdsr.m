function makerf_bdsr(sel_data,nc,station)
...
if exist([sel_data.save_dir '/' sel_data.nc '.' sel_data.station '-' sel_data.vpsfile],'file')
    fid = fopen([sel_data.save_dir '/' sel_data.nc '.' sel_data.station '-' sel_data.vpsfile],'rt');
    C = textscan(fid,'%f %f %f','Delimiter',',');
    fclose(fid);
    freq = C{1,1};
    vst = C{1,2};
    dep = C{1,3};
    [~,ioi] = min(abs(freq-sel_data.fcut));
    vsf = vst(ioi);
    vpsflag = 1;
    % stat = C{1,1};
    % temp = C{1,2};
...
if ~vpsflag
    vsf = findsurfacevp2(sel_data,data,fn,mis);
end

disp("Calculate receiver functions")
% z_targets = [-50,0,50,100,150,200]; % Targeting Depth
z_targets = linspace(-100,500,30);
if sum(z_targets==0) == 0
    z_targets(end+1) = 0;
    z_targets = sort(z_targets);
end
% 1-D velocity model for mapping to depth
ldepth = [20, 35, 300, 600]; % depths of interfaces
velp = [5.8, 6.5, 8, 9]; % P wave velocity
vels = [3.4, 3.75, 4.5, 5]; % S wave velocity
...
for iev = 1:length(fn)
    ...
    % end
    % keyboard
    [incl] = get_theo_inc(baz,rayp,vsf.*1.73,vsf,3);
