function [hr,ht,stdevr,stdevt] = MTCv2(phase,tshift,sel_data)
...
for i = 1:length(phase.tr)
    ...
    for kk = 1:nwin
        ...
        fftdum = fft(datapad);
        if strcmp(phase.comp(i),comp(1))
            pfftz(1:nf,nwin+kk) = fftdum(1:nf);
        elseif strcmp(phase.comp(i),comp(2))
            pfftr(1:nf,nwin+kk) = fftdum(1:nf);
        elseif strcmp(phase.comp(i),comp(3))
            pfftt(1:nf,nwin+kk) = fftdum(1:nf);
        end
    end
end
...
for i = 1:length(phase.tr)
    ...
    for kk = 1:nwin
        ...
        fftdum = fft(datapad);
        if strcmp(phase.comp(i),comp(1))
            fftz(1:nf,nwin+kk) = fftdum(1:nf);
        elseif strcmp(phase.comp(i),comp(2))
            fftr(1:nf,nwin+kk) = fftdum(1:nf);
        elseif strcmp(phase.comp(i),comp(3))
            fftt(1:nf,nwin+kk) = fftdum(1:nf);
        end
    end
end
