function bdsr_prepHD2Dpick(path2opts,sel_data)
...
sel_data.load_dir = [sel_data.work_dir '/hd/'];
sel_data.save_dir = [sel_data.work_dir '/hd/'];
% if ~exist(sel_data.save_dir)
%     mkdir(sel_data.save_dir);
% end

sel_data.origflag = input('Use raw(0) or weighted(1) harmonics? (0/1): ');

prephd2dpick_bdsr(sel_data);
